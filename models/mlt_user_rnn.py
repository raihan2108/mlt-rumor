    ...
    def construct_vae(self):
        with tf.variable_scope('user-vae'):
            ...
            kl_divergence = -0.5 * tf.reduce_sum(
                1 + self.logvar_encoder - tf.square(mu_encoder) - tf.exp(self.logvar_encoder),
                axis=2)
            bce = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.x_hat, labels=self.user_feat),
                                axis=2)
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('rumor-clf'):
            if self.cell_type == 'lstm':
                ...
                self.temp_score = tf.layers.dense(inputs=self.input_feat_rumor, units=64,
                    activation=None, kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                                   bias_initializer=tf.constant_initializer(0.25))
                self.rumor_score = tf.layers.dense(inputs=self.temp_score, units=self.r_label_size,
                    activation=None, kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                                   bias_initializer=tf.constant_initializer(0.25))
            else:
        ...
        with tf.variable_scope('stance-clf'):
            ...
            self.stance_rnn_output, self.stance_hidden_state = \
                tf.nn.dynamic_rnn(self.rnn_cell_stance, self.input_feat, self.seq_len, self.init_state_stance)

            output = tf.reshape(self.stance_rnn_output, [-1, self.state_size])
        ...
        if self.arch == 'joint':
            with tf.variable_scope('joint-opt'):
                ...
                self.total_cost = self.rumor_label_cost + self.vae_loss + tf.constant(
                    1.0) * self.stance_label_cost  # + self.reg_coeff * self.regularization_cost
        ...
        with tf.Session() as sess:
            ...
            for epi in range(1, self.epochs + 1):
                ...
                for i in range(0, n_batches):
                    ...
                    if self.arch == 'joint':
                        ...
                        _, total_cost, rumor_cost, stance_cost = sess.run([self.train_op, self.total_cost,
                                                                           self.rumor_label_cost,
                                                                           self.stance_label_cost], feed_dict=fd_all)
                        #  print(vae_cost)
                ...
                if epi % self.display_interval == 0:
                    if self.arch == 'joint':
                        print(
                            'end of epoch: {} avg total cost: {:0.3f} avg rumor cost: {:0.3f} avg stance cost: {:0.3f}'.
